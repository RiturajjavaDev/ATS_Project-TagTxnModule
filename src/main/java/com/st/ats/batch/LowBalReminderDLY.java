...
/**
 *This batch class is used to send email and message to user whose tag balance is going in
 *negative or low balance 
 * 
 * @author Rituraj
 *
 */
		...
		failTxnTriggerEntities.forEach(failEntity->{
			...
			Long tagId=failEntity.getTagId();
			 ...
			 if(userInfo.isPresent()) {
				 ...
				 this.process(userEntity.getEmail(),userEntity.getPhno(),txnId,userEntity.getFname(),userEntity.getLname());
